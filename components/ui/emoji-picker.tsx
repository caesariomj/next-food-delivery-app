...
function EmojiPickerSearch({
  ...
  return (
    <div
      className={cn("flex h-9 items-center gap-2 border-b-4 px-3", className)}
...
function EmojiPickerCategoryHeader({
  ...
  return (
    <div
      {...props}
      className="bg-popover px-3 pt-3.5 pb-2 text-sm leading-none text-muted-foreground"
...
function EmojiPickerFooter({
  ...
  return (
    <div
      className={cn(
        "flex w-full max-w-(--frimousse-viewport-width) min-w-0 items-center gap-1 border-t-4 p-2",
        className
      )}
      ...
      <EmojiPickerPrimitive.ActiveEmoji>
        {({ emoji }) =>
          emoji ? (
            ...
            <span className="ml-1.5 flex h-7 items-center truncate text-sm text-muted-foreground">
              Select an emoji…
            </span>
          )
        }
